...
class Composition(dict):
    """
    Representation of a single composition.
    Expected to be Loaded from a music XML leadsheet.
    Attribute get and set modified to write directly to the database.
    """
    ...
    def __setattr__(self, attr, value):
        ...
        if '_Composition__initialized' not in self.__dict__:
            return dict.__setattr__(self, attr, value)
        elif attr in self.__dict__: # instance variables are maintained as-is
            dict.__setattr__(self, attr, value)
        else:
            self.__dict__['record'][attr] = value
            self.db.compositions.update_one(
                {'_id': self.record['_id']}, {'$set': self.record})
    ...
    def xmldom(self):
        """
        Return the XML of this composition.
        """
        if not self.xml:
    ...
    def analyze(self, analysis_name):
        """
        Run the analysis by the name |analysis_name| on this composition.
        """
        try:
            analysis_module = importlib.import_module(
                'leadreader.analyses.' + analysis_name)
        except ImportError:
